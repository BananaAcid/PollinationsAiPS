...
<#
    .SYNOPSIS
    Generate Audio using the Pollinations AI API.

    .DESCRIPTION
    Generate audio based on the given prompt using the Pollinations AI API.
    ...
    .PARAMETER out
    The local path to save the generated audio.
    
    .PARAMETER save
    Will save to the system temp folder.

    .PARAMETER details
    Will return the details of the generated audio (headers + content).
    ...
    List the available models, then generate an audio based on the prompt "a cat" and save it.

    .EXAMPLE
    PS C:\> Get-PollinationsAiAudio "a cat" -set @{"system" = "just output a comma separated list of typical colors"}

    Generate an audio based on the prompt "a cat" and set the system prompt to "just output a comma separated list of typical colors".

    .EXAMPLE
    PS C:\> $env:POLLINATIONSAI_API_KEY = "sk_..."
    PS C:\> $s = Get-PollinationsAiAudio -getSettingsDefault
    PS C:\> $s
        Name                           Value
        ----                           -----
        system
    ...
    PS C:\> Get-PollinationsAiAudio -content "a cat" -settings $s -out acat.jpg
...
#>
Function Get-PollinationsAiAudio {
    ...
    $querySettings = @{
        "model" = if (-not $model) { "openai-audio" } else { $model } # since this could be set to ""
    ...
    if ($out -ne "" -or $save -eq $true) {
        ...
        if ($Null -eq (Split-Path $filepath -Leaf).Split(".")[1]) { #PWSH 6+ "" -eq (Split-Path $filepath -Leaf | Split-Path -Extension)
            ...
            $type = $response.Headers["Content-Type"] -split ";" | select -First 1 |% { $_ -split "/"}
            $ext = $type[1]
            if ($type -eq "") { $ext = "mpeg" } #! default fallback
            if ($type[1] -eq "plain") { $ext = "txt" }
            if ($type[0] -eq "audio" -and $type[1] -eq "mpeg") { $ext = "mp3" }
            $filepath += "." + $ext
        }
